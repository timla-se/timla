...
def upgrade() -> None:
    op.execute("""
        ...
        CREATE TABLE staff (
            ...
            created_at timestamptz NOT NULL DEFAULT now(),
            -- Referenced by the composite FKs below so rows in shift and
            -- availability_interval can never point at another org's staff.
            CONSTRAINT staff_id_org_unique UNIQUE (id, org_id)
        );
        ...
        CREATE TABLE shift (
            ...
            -- Same-org guarantee; deleting staff nulls only staff_id
            -- (PG15+ column list), leaving an open shift in the right org.
            CONSTRAINT shift_staff_same_org
        ...
        CREATE TABLE availability_interval (
            ...
            CONSTRAINT availability_staff_same_org
                FOREIGN KEY (staff_id, org_id) REFERENCES staff (id, org_id)
                ON DELETE CASCADE
        );
        ...
        CREATE TABLE publication (
            ...
            CONSTRAINT publication_week_format CHECK (
                week ~ '^\\d{4}-W\\d{2}$'
